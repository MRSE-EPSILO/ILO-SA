...
```{r}
microcuencas <- read_sf("shp/cuencas.shp") %>% 
  as_Spatial()

estacion <- read_sf("shp/estación.shp") %>% 
  as_Spatial()

qocha <- read_sf("shp/qocha.shp") %>% 
  as_Spatial()
...
leaflet() %>% 
  ...
  addPolygons(
    data = microcuencas,
    popup = "Unidad Hidrológica",
    fillColor = "skyblue",
    color = "white",
    label = ~nombre, group = "Cuencas") %>% 
  addMarkers(data = estacion,
             popup = "Estación pluviométrica",
             group = "Pluviómetro") %>% 
  addMarkers(data = qocha,
             popup = "Qocha",
             group = "Qocha") %>% 
...
```{r}
xaringanExtra::embed_xaringan("m2/pres.html",ratio="4:3")
```
...
<iframe width="100%" height="450" frameborder="0" title="Felt Map" src="https://felt.com/embed/map/ASANA-zpj9BLDoUSDaneE59AUwK9APD?loc=-17.04423,-70.49779,13.14z"></iframe>
